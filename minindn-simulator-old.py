...
def main():
    ...
    pro_names = [host.name for host in ndn.net.hosts if host.name.startswith('pro')]
    con_names = [host.name for host in ndn.net.hosts if host.name.startswith('con')]
    total_host = len(pro_names)
    ok_file = [os.path.join(TEMP_DIR, f"pro{idx}.ok") for idx in range(total_host)]

    for name in pro_names + con_names:
        grh.addOrigin([ndn.net[name]], [f"/{name}"])
    ...
    if os.path.isfile(CLIENT_BIN):
        for first in range(total_host):
            pro_name = f'pro{first}'
            producer = ndn.net[pro_name]
            # 保留参数位置，用户自行填充
            cmd = (
            ...
            info(f'start client on {pro_name}: {cmd}\n')
            producer.cmd(cmd)
    else:
    ...
    if (os.path.exists(os.path.join(TEMP_DIR, "all.finish"))):
